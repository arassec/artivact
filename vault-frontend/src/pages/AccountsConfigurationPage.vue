<template>
  <ArtivactContent>
    ...
    <div class="col">
      ...
      <div class="q-mb-lg">All system accounts can be configured on this page.</div>
            ...
            <q-item>
              ...
              <q-item-section side>
                <q-btn round dense flat class="float-right" color="white"
                       icon="delete" size="md" @click="deleteAccount(index)"></q-btn>
              </q-item-section>

              <div class="float-right">
                <q-btn round dense flat icon="edit" size="md"
                       @click="editAccountRef = account; showEditModalRef = true"></q-btn>
                <q-btn round dense flat icon="delete" size="md"
                       @click="deleteAccountIdRef = account.id; showDeleteModalRef = true"></q-btn>
              </div>
            </q-item>
      ...
      <q-btn label="Add Account" @click="showCreateModalRef = true" color="primary" class="float-right q-mb-lg"/>
    </div>
    ...
    <q-dialog v-model="showCreateModalRef" persistent>
      <q-card class="q-mb-lg artivact-modal-content">
        ...
        <q-card-section>
          <q-input outlined v-model="usernameRef" class="col-5 q-mb-lg" type="text" name="username" label="Username"/>
          ...
          <q-input outlined v-model="passwordRepeatRef" class="col-5" type="password" name="password"
                   label="Password (repeat)" :error="passwordValidationFailedRef"/>

          <q-input outlined v-model="emailRef" class="col-5 q-mb-md" type="text" name="email"
                   label="E-Mail"/>

          <div>
            <q-checkbox v-model="userRef" class="col-5" name="user" label="User?"/>
          </div>

          <div>
            <q-checkbox v-model="adminRef" class="col-5" name="admin" label="Admin?"/>
          </div>
    ...
    <q-dialog v-model="showEditModalRef" persistent v-if="editAccountRef" class="artivact-modal">
      <q-card class="q-mb-lg artivact-modal-content">
        <q-card-section class="bg-primary text-white">
        ...
        <q-card-section>
          <q-input outlined v-model="editAccountRef.username" class="col-5 q-mb-lg" type="text" name="username"
                   label="Username"/>

          <q-input outlined v-model="editAccountRef.password" class="col-5" type="password" name="password"
                   label="Password"
                   :error="passwordValidationFailedRef"/>
          ...
          <q-input outlined v-model="editAccountRef.email" class="col-5 q-mb-md" type="text" name="email"
                   label="E-Mail"/>

          <div>
            <q-checkbox v-model="editAccountRef.user" class="col-5" name="user" label="User?"/>
          </div>

          <div>
            <q-checkbox v-model="editAccountRef.admin" class="col-5" name="admin" label="Admin?"/>
          </div>
    ...
    <q-dialog v-model="showDeleteModalRef" persistent class="artivact-modal">
      <q-card class="artivact-modal-content">
        <q-card-section class="row items-center">
          <q-icon name="warning" size="md" color="warning" class="q-mr-md"></q-icon>
          <h3 class="av-text-h3">Delete Account?</h3>
          <div class="q-ml-sm"> Are you sure you want to delete this account? This action cannot be undone!</div>
        </q-card-section>

        <q-card-section>
          <q-btn flat label="Cancel" color="primary" v-close-popup/>
          <q-btn flat label="Delete Account" color="primary" v-close-popup @click="deleteAccount(deleteAccountIdRef)"
                 class="float-right"/>
        </q-card-section>
      </q-card>
    </q-dialog>
...
function loadAccounts() {
  api.get('/api/administration/account')
    .then((response) => {
...
function createAccount(): boolean {
  ...
  api.post('/api/administration/account', account)
    .then((response) => {
...
function deleteAccount(id: number) {
  api.delete('/api/administration/account/' + id)
    .then(() => {
      loadAccounts()
    })
    .catch(() => {
